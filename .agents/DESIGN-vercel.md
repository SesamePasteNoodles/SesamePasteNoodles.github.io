...
> **Reference only.** This document records visual observations about Vercel and is not the
> implementation specification for this portfolio. Project-specific decisions—including the
> mustard-yellow identity, narrow desktop rail, dark Tech Stack band, typography for Traditional
> Chinese, accessibility requirements, and component behavior—are defined in
> [`planning.md`](./planning.md). Delivery order and acceptance criteria are defined in
> [`IMPLEMENTATION-PLAN.md`](./IMPLEMENTATION-PLAN.md).

## Project Adoption Matrix

| Reference characteristic | Portfolio decision |
|---|---|
| Near-white canvas, dark ink, hairline borders | Adopt |
| Clear type hierarchy and generous whitespace | Adopt |
| Geist Sans and Geist Mono | Adopt for Latin and technical labels; use the Chinese stack from `planning.md` |
| Precise card grids | Adapt into a dark Bento Grid and project-led layouts |
| Minimal elevation | Adopt; use shadow mainly for interaction feedback |
| Hero mesh gradient | Replace with a restrained mustard-yellow glow and local grid |
| Color confined to the hero | Reject; mustard yellow remains a controlled identity across the page |
| Generic SaaS navigation and CTA patterns | Adapt to the portfolio's top navigation and narrow section rail |
| Vercel product bands, pricing and logo strips | Do not use unless required by portfolio content |
...
#### Touch Targets
The extracted visual reference does not prove an accessible hit-area size. For this portfolio,
marketing CTAs and navigation controls must explicitly define a minimum 44px interaction height;
line-height alone is not treated as sufficient evidence.
...
> The reference material does not document complete hover and focus states. Do not infer them from
> this file; use the project interaction and accessibility rules in `planning.md`.
